...
class MediaProcessor:
    ...
    @staticmethod
    def validate_and_process_image(content: bytes, mime_type: str) -> Dict[str, Union[int, float]]:
        """
        ...
        try:
            ...
            raise MediaValidationError(f"OpenCV error processing image: {str(e)}")
        except Exception as e:
            raise MediaValidationError(f"Error processing image: {str(e)}")
    
    @staticmethod
    def validate_and_process_video(content: bytes, mime_type: str) -> Dict[str, Union[int, float]]:
        """
        ...
        try:
            ...
            raise MediaValidationError(f"OpenCV error processing video: {str(e)}")
        except Exception as e:
            raise MediaValidationError(f"Error processing video: {str(e)}")
    
    @classmethod
    def process_upload(cls, file: UploadFile, content: bytes) -> Dict[str, Union[str, int, float, None]]:
        """
        ...
        try:
            ...
            logger.warning(f"Media validation failed for {file.filename}: {str(e)}")
